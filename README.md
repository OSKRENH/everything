...
```bash
npm install                # ставит зависимости для server и client разом
cp server/.env.example server/.env
# откройте server/.env и задайте случайный JWT_SECRET
npm run dev                 # поднимает backend (:4000) и frontend (:5173) параллельно
```

Откройте http://localhost:5173, зарегистрируйтесь, добавьте продукты/технику на странице
«Инвентарь» и перейдите в «Рецепты».
