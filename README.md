...
Это нужно сделать один раз вручную в вашем Cloudflare-аккаунте — я не могу выполнить эти шаги
за вас, так как для них требуется вход в ваш аккаунт.

1. **Создайте бесплатный аккаунт** на https://dash.cloudflare.com, если его ещё нет.
2. **Авторизуйте wrangler** на своём компьютере (откроется браузер для входа):
   ```bash
   npx wrangler login
   ```
3. **Создайте продовую базу D1:**
   ```bash
   npx wrangler d1 create kutno-db
   ```
   Команда выведет `database_id` — скопируйте его в `wrangler.toml`, заменив
   `REPLACE_WITH_YOUR_D1_DATABASE_ID`, и закоммитьте это изменение (сам `database_id` не секрет).
4. **Примените схему к продовой базе:**
   ```bash
   npm run db:migrate:remote
   ```
5. **Подключите GitHub-репозиторий в Cloudflare Pages** (даёт автодеплой на каждый push):
...
6. **Привяжите базу D1 к проекту Pages:**
...
7. **Добавьте секреты окружения:**
   - **Settings → Environment variables** → добавьте `JWT_SECRET` и `SPOONACULAR_API_KEY`,
     отметьте их как **Encrypt (secret)**.
8. **Пересоберите проект**, чтобы новые привязки и переменные подхватились: **Deployments** →
