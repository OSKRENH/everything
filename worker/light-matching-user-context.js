import { ingredientMatchAny, normalizeIngredient } from "./light-ingredient-semantics.js";
...
function parseMatchingAmount(value = "") {
...
function matchingPantryItem(ingredient, pantry, semantics) {
  return Object.values(pantry).find((item) => ["exact", "category"].includes(ingredientMatchAny(ingredient, item.name, semantics).type)) || null;
}

function quantityAssessmentForAnalysis(analysis, pantryValue, semantics) {
  ...
  for (const ingredient of analysis?.ingredients || []) {
    ...
    const pantryItem = matchingPantryItem(ingredient, pantry, semantics);
...
function feedbackIngredientMatch(recipe, ingredient, semantics) {
  return (recipe?.ingredients || []).some((item) => ingredientMatchAny(item, ingredient, semantics).type !== "none");
}

function sharedFeedbackIngredients(recipe, feedback, semantics) {
  const target = feedback.ingredients || [];
  return target.filter((name) => feedbackIngredientMatch(recipe, name, semantics)).length;
}

function preferencePenaltyForRecipe(recipe, feedbackValue, semantics, now = Date.now()) {
  const feedback = sanitizeMatchingFeedback(feedbackValue).filter((item) => now - item.at <= FEEDBACK_WINDOW_MS);
  let penalty = 0;
  for (const item of feedback) {
    if (item.reason === "dislike-ingredient" && item.ingredient && feedbackIngredientMatch(recipe, item.ingredient, semantics)) penalty += 170;
    ...
    if (item.reason === "more-like-this") {
      ...
      penalty -= Math.min(36, sharedFeedbackIngredients(recipe, item, semantics) * 12);
    }
  }
  return penalty;
}
...
export function applyMatchingUserContext(recipe, baseAnalysis, context = {}, semantics = {}) {
  const assessment = quantityAssessmentForAnalysis(baseAnalysis, context.pantry || {}, semantics);
  ...
  const preferencePenalty = preferencePenaltyForRecipe(recipe, context.feedback || [], semantics);
  const reasons = (baseAnalysis.reasons || []).filter((reason) => !assessment.low.length || !/все обязательные продукты/i.test(reason));
  if (assessment.low.length) reasons.unshift(...assessment.low.slice(0, 2).map((item) => `Не хватает количества: ${item.name} — есть ${item.have.quantity} ${item.have.unit}, нужно ${item.need}`));
  const score = Number(baseAnalysis.score || 0) - groupBase(baseAnalysis.group) + groupBase(group) - assessment.low.length * 55 - preferencePenalty;
